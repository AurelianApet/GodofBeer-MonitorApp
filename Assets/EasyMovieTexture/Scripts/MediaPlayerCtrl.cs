...
public class MediaPlayerCtrl : MonoBehaviour
{
	...
	public GameObject[] m_TargetMaterial = new GameObject[1];
    ...
    private bool m_bBuffring = false;
	...
	void Start()
	{
        ...
        m_video = this.gameObject.AddComponent<VideoPlayer>();
        m_video.playOnAwake = false;
        m_video.renderMode = VideoRenderMode.APIOnly;
#endif
	...
	Call_SetUnityTexture (m_VideoTexture.GetNativeTextureID ());
				...
				if (OnResize != null)
					OnResize ();
                if(m_eventOnResize != null)
                    m_eventOnResize.Invoke();
			} else {
            ...
            if( m_iCurrentSeekPosition  == Call_GetSeekPosition())
            {
                if(m_bBuffring == false)
                {
                    m_bBuffring = true;
                    if(OnVideoBuffering != null)
                        OnVideoBuffering();

                    if (m_eventOnVideoBuffering != null)
                        m_eventOnVideoBuffering.Invoke();
                }
            ...
            else
            {
                if(m_bBuffring == true)
                {
                    if(OnVideoBufferingEnd != null)
                        OnVideoBufferingEnd();

                    if (m_eventOnVideoBufferingEnd != null)
                        m_eventOnVideoBufferingEnd.Invoke();
                }
                m_bBuffring = false;
            }
		...
		if (m_CurrentState != Call_GetStatus())
		{
			...
			if (m_CurrentState == MEDIAPLAYER_STATE.READY)
			{
                ...
                else
                {
                    ...
                    if (m_eventOnReady != null)
                        m_eventOnReady.Invoke();
	...
	void OnError(MEDIAPLAYER_ERROR iCode, MEDIAPLAYER_ERROR iCodeExtra)
	{
        ...
        if (m_eventOnVideoError != null)
            m_eventOnVideoError.Invoke();
	...
	{
#if UNITY_EDITOR || UNITY_STANDALONE 
        return (Texture2D)m_video.texture;
#else
	...
	public void Play()
	{
        ...
        if (m_CurrentState == MEDIAPLAYER_STATE.PAUSED) 
		{
            ...
            Call_RePlay ();
		}else if ( m_CurrentState == MEDIAPLAYER_STATE.STOPPED)
        {
            Load(m_strFileName);
        }
		...
		else if (m_CurrentState == MEDIAPLAYER_STATE.NOT_READY) 
		{
            ...
            if (string.IsNullOrEmpty(m_strFileName) == false)
            {
	...
	public void SeekTo(int iSeek)
	{
        ...
        if (m_CurrentState == MEDIAPLAYER_STATE.PLAYING || m_CurrentState == MEDIAPLAYER_STATE.READY || m_CurrentState == MEDIAPLAYER_STATE.PAUSED )
    ...
    if (m_eventOnVideoFirstFrameReady != null)
    m_eventOnVideoFirstFrameReady.Invoke();
	}
	}
	...
	private  bool Call_Load(string strFileName, int iSeek)
	{
        ...
        {
            m_video.url = strFileName;
        }
        else
        {
            m_video.url = Application.streamingAssetsPath + "/" + strFileName;
        }

        Debug.Log(m_video.url);
        
        m_video.renderMode = UnityEngine.Video.VideoRenderMode.APIOnly;



       
        m_video.Prepare();
        m_video.prepareCompleted += OnVideoPrepared;
    ...
    void OnVideoPrepared(VideoPlayer source_)
    {
        for (int i = 0; i < m_TargetMaterial.Length; i++)
        {
            if (m_TargetMaterial[i])
            {
                if (m_TargetMaterial[i].GetComponent<MeshRenderer>() != null)
                {
                    if (m_TargetMaterial[i].GetComponent<MeshRenderer>().material.mainTexture != source_.texture)
                    {
                        m_TargetMaterial[i].GetComponent<MeshRenderer>().material.mainTexture = source_.texture;
                    }
                }

                if (m_TargetMaterial[i].GetComponent<RawImage>() != null)
                {
                    if (m_TargetMaterial[i].GetComponent<RawImage>().texture != source_.texture)
                    {
                        m_TargetMaterial[i].GetComponent<RawImage>().texture = source_.texture;
                    }
                }
        ...
        m_iWidth = source_.texture.width;
        m_iHeight = source_.texture.height;

        


        if (m_video.clip != null)
            Debug.Log(m_video.clip.length);
	...
	private void Call_SetVolume(float fVolume)
	{
        m_video.SetDirectAudioVolume(0, fVolume);
	...
	private void Call_SetSeekPosition(int iSeek)
	{
        ...
        m_video.time = ((double)iSeek / 1000.0);

    }

	private int Call_GetSeekPosition()
	{
		return (int)(m_video.time * 1000.0);
	}

	private void Call_Play(int iSeek)
	{

		if (m_CurrentState == MEDIAPLAYER_STATE.READY || m_CurrentState == MEDIAPLAYER_STATE.END  || m_CurrentState == MEDIAPLAYER_STATE.PAUSED) {
	...
	private int Call_GetVideoWidth()
	{
        ...
        return m_video.texture.width;
	}

	private int Call_GetVideoHeight()
	{
       
        return m_video.texture.height;
	}
	...
	private int Call_GetDuration()
	{
        ...
        return (int) (m_video.frameCount / m_video.frameRate * 1000.0);
    }
